...
st.header("⚠️ Risk Analysis Tender")
...
with col1:
    uploaded_logo = st.file_uploader("Upload company logo (PNG/JPG)", type=["png", "jpg", "jpeg"], key="logo_risk")
    if uploaded_logo is not None:
        ...
        if not os.path.exists("assets/logo.png"):
            ...
            if st.button("Replace global default logo", key="replace_logo_risk"):
                with open("assets/logo.png", "wb") as f:
                    f.write(logo_bytes)
...
if df.empty:
    ...
    st.subheader("📋 Klasifikasi Risiko Tender")
    ...
    st.download_button("⬇️ Download CSV", data=csv_bytes, file_name="risk_report.csv", mime="text/csv")
    ...
    try:
        ...
        st.download_button("📄 Download PDF", data=pdf_bytes, file_name="risk_report.pdf", mime="application/pdf")
